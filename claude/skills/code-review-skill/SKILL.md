...
1. **Determine the target branch** — detect the platform from `git remote get-url origin`, then query the open MR/PR:
   - GitHub (`github.com`): `gh pr view --json baseRefName`
   - GitLab (`gitlab.com` or self-hosted): `glab mr view --output json`
   - If neither CLI is available or no open MR/PR exists, ask the user.
   - Never assume `main`/`master` or any other branch — always confirm the actual merge target and use it as the base for all `git diff` commands.
2. Read PR/MR description and linked issue
3. Check PR size (>400 lines? Ask to split)
4. Review CI/CD status (tests passing?)
5. Understand the business requirement
6. Note any relevant architectural decisions

**If reviewing local changes (no PR/MR):**
1. **Determine what to diff** — ask the user if unclear, otherwise use:
   - `git diff` — unstaged changes
   - `git diff --cached` — staged changes
   - `git diff HEAD` — all local changes (staged + unstaged)
   - `git diff <branch>...HEAD` — commits on current branch not yet on `<branch>`
2. Run the appropriate `git diff` and check the size (>400 lines? consider focusing the review)
3. Note any relevant context the user provides about intent or constraints
