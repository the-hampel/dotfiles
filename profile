...
if [ "$HOSTNAME" = thinkxtreme ]; then
    ...
    alias getroc='srun --nodes=1 --time 12:00:00 --partition=porgy05 --ntasks-per-node=2 --cpus-per-task=10 --cpu-bind=cores --gres=gpu:2 --pty bash -i'
    alias getintel='srun --nodes=1 --time 12:00:00 --partition=guppy07 --ntasks-per-node=2 --cpus-per-task=10 --cpu-bind=cores --pty bash -i'
